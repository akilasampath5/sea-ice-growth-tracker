...
!echmod      save
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block,max_blocks), save :: &
         ...
         TTLON    ! longitude of temp pts (radians)
         
      integer (kind=int_kind), dimension(nx_block,ny_block,max_blocks), save :: &
         thicmsk_N

      real (kind=dbl_kind), dimension(18,nx_block,ny_block,max_blocks), save :: &
         tmsk_N
...
!
      ...
      real (kind=dbl_kind), dimension(7) :: bzlat
      real (kind=dbl_kind), dimension(3) :: bzlon
      real (kind=dbl_kind), dimension(11) :: bzlon1
      real (kind=dbl_kind), dimension(7) :: bzlon2
      real (kind=dbl_kind), dimension(6) :: bzlow
      real (kind=dbl_kind), dimension(6) :: bzlow1
      ...
      !-----------------------------------------
            ...
            data bzlat/ &
                    !SH polar region
              -50.0_dbl_kind, -65.0_dbl_kind, -90.0_dbl_kind, &!65-69-80-85
                    !NH polar region
              45.0_dbl_kind, 65.0_dbl_kind, 80.0_dbl_kind, &!65-69-80-85
               90.0_dbl_kind/

!            data bzlon/ &
!                -180.0_dbl_kind, -130.0_dbl_kind, -70.0_dbl_kind, &
!!                -50.0_dbl_kind, 0.0_dbl_kind,  &
!                130.0_dbl_kind, 180.0_dbl_kind/ 

            data bzlon1/ &
                -180.0_dbl_kind, -150.0_dbl_kind,& 
                -115.0_dbl_kind, &
                -85.0_dbl_kind,   &
                -45.0_dbl_kind,  &
                 0.0_dbl_kind,     &
                 30.0_dbl_kind,  &
                 45.0_dbl_kind,  102.0_dbl_kind, &
                 150.0_dbl_kind, & 
                 180.0_dbl_kind/
            ...
            data bzlon2/ &
                -180.0_dbl_kind, -96.0_dbl_kind, & 
                 -68.0_dbl_kind,                  &
                 -44.0_dbl_kind,-20.0_dbl_kind,   &
                 40.0_dbl_kind,                   &
                 180.0_dbl_kind/
!                 90.0_dbl_kind,180.0_dbl_kind/
...
!                -60.0_dbl_kind,  &
!                 0.0_dbl_kind,     &
!                 40.0_dbl_kind, 60.0_dbl_kind, &
!                 100.0_dbl_kind,  &
!                 140.0_dbl_kind, & 
!                 160.0_dbl_kind, & 
!                 180.0_dbl_kind/
            data bzlow/ &
!              -180.0_dbl_kind, -170.0_dbl_kind, -160.0_dbl_kind, &!65-69-80-85
!              -150.0_dbl_kind, -140.0_dbl_kind, -130.0_dbl_kind, &!65-69-80-85
!              -120.0_dbl_kind, -90.0_dbl_kind, -80.0_dbl_kind, &!65-69-80-85
               0.0_dbl_kind, &
              40.0_dbl_kind, &!65-69-80-85
              80.0_dbl_kind, 120.0_dbl_kind, &!65-69-80-85
              160.0_dbl_kind, &!65-69-80-85
              180.0_dbl_kind/ 

            data bzlow1/ &
              -180.0_dbl_kind, -160.0_dbl_kind, &!65-69-80-85
              -120.0_dbl_kind, -80.0_dbl_kind, &
               -40.0_dbl_kind, 0.0_dbl_kind/!65-69-80-85
...
!        print *,'TLON',TLON(i,j,iblk)*rad_to_deg
!        print *,'TLAT',TLAT(i,j,iblk)*rad_to_deg
!           if ((TLAT(i,j,iblk)*rad_to_deg.ge.70.0_dbl_kind            &
!                 .and. TLAT(i,j,iblk)*rad_to_deg.lt.80.0_dbl_kind)    &
!              .and. (TLON(i,j,iblk)*rad_to_deg.ge.0.0_dbl_kind .and.&
!            TLON(i,j,iblk)*rad_to_deg.lt.50.0_dbl_kind)&
!                    .and. (hm(i,j,iblk) .ne. c0)) then
!
!                    thicmsk_N(i,j,iblk) = 1

!           end if
              do ind = 1,1
                    if ((TLAT(i,j,iblk)*rad_to_deg.ge.bzlat(4)  &
                     .and. TLAT(i,j,iblk)*rad_to_deg.lt.bzlat(5)) &
                     .and. (TLON(i,j,iblk)*rad_to_deg.ge.bzlon2(ind) .and.&
                       TLON(i,j,iblk)*rad_to_deg.lt.bzlon2(ind+1))&
                       .and. (hm(i,j,iblk) .ne. c0)) then
                            thicmsk_N(i,j,iblk) = ind 
                            tmsk_N(ind,i,j,iblk) = 1 
                    end if
              end do
              do ind = 2,2
                    if ((TLAT(i,j,iblk)*rad_to_deg.ge.bzlat(4)  &
                     .and. TLAT(i,j,iblk)*rad_to_deg.lt.70.0_dbl_kind) &
                     .and. (TLON(i,j,iblk)*rad_to_deg.ge.bzlon2(ind) .and.&
                       TLON(i,j,iblk)*rad_to_deg.lt.bzlon2(ind+1))&
                       .and. (hm(i,j,iblk) .ne. c0)) then
                            thicmsk_N(i,j,iblk) = ind 
             ...
             do ind = 1,3
                    if ((TLAT(i,j,iblk)*rad_to_deg.ge.bzlat(5)  &
                     .and. TLAT(i,j,iblk)*rad_to_deg.lt.bzlat(6)) &
                     .and. (TLON(i,j,iblk)*rad_to_deg.ge.bzlon1(ind) .and.&
                       TLON(i,j,iblk)*rad_to_deg.lt.bzlon1(ind+1))&
                       .and. (hm(i,j,iblk) .ne. c0)) then
                            thicmsk_N(i,j,iblk) = ind+6 
                            tmsk_N(ind+6,i,j,iblk) = 1 
                    end if
             end do
             do ind = 4,4
                    if ((TLAT(i,j,iblk)*rad_to_deg.ge.70.0_dbl_kind  &
             ...
             do ind = 1,2
                    if ((TLAT(i,j,iblk)*rad_to_deg.ge.bzlat(6)  &
                    .and. TLAT(i,j,iblk)*rad_to_deg.lt.bzlat(7)) &
                     .and. (TLON(i,j,iblk)*rad_to_deg.ge.bzlon(ind) .and.&
                      TLON(i,j,iblk)*rad_to_deg.lt.bzlon(ind+1))&
                       .and. (hm(i,j,iblk) .ne. c0)) then
                            thicmsk_N(i,j,iblk) = ind+16
                            tmsk_N(ind+16,i,j,iblk) = 1
                    end if
             end do
!cutitout
!             do ind = 1,5
!                    if ((TLAT(i,j,iblk)*rad_to_deg.le.-65)  &
!                     .and. (TLON(i,j,iblk)*rad_to_deg.ge.bzlow(ind) .and.&
!                      TLON(i,j,iblk)*rad_to_deg.lt.bzlow(ind+1))&
!                       .and. (hm(i,j,iblk) .ne. c0)) then
!                            thicmsk_N(i,j,iblk) = ind+19
!                            tmsk_N(ind+19,i,j,iblk) = 1
!                    end if
!             end do


!             do ind = 1,5
!                    if ((TLAT(i,j,iblk)*rad_to_deg.le.-50  &
!                    .and. TLAT(i,j,iblk)*rad_to_deg.gt.-65) &
!                     .and. (TLON(i,j,iblk)*rad_to_deg.ge.bzlow(ind) .and.&
!                      TLON(i,j,iblk)*rad_to_deg.lt.bzlow(ind+1))&
!                       .and. (hm(i,j,iblk) .ne. c0)) then
!                            thicmsk_N(i,j,iblk) = ind+24
!                            tmsk_N(ind+24,i,j,iblk) = 1
!                    end if
!             end do
!!
!             do ind = 1,5
!                    if ((TLAT(i,j,iblk)*rad_to_deg.le.-70)  &
!                     .and. (TLON(i,j,iblk)*rad_to_deg.ge.bzlow1(ind) .and.&
!                      TLON(i,j,iblk)*rad_to_deg.lt.bzlow1(ind+1))&
!                       .and. (hm(i,j,iblk) .ne. c0)) then
!                            thicmsk_N(i,j,iblk) = ind+29
!                            tmsk_N(ind+29,i,j,iblk) = 1 
!                    end if
!             end do
!             do ind = 1,5
!                    if ((TLAT(i,j,iblk)*rad_to_deg.le.-50  &
!                    .and. TLAT(i,j,iblk)*rad_to_deg.gt.-70) &
!                     .and. (TLON(i,j,iblk)*rad_to_deg.ge.bzlow1(ind) .and.&
!                      TLON(i,j,iblk)*rad_to_deg.lt.bzlow1(ind+1))&
!                       .and. (hm(i,j,iblk) .ne. c0)) then
!                            thicmsk_N(i,j,iblk) = ind+34
!                            tmsk_N(ind+34,i,j,iblk) = 1
!                    end if
!             end do
