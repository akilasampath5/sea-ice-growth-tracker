...
!
      ...
      use ice_state, only: nt_Tsfc, nt_iage, nt_thickchng, nt_FY, &
                           nt_volpn, nt_aero,                     &
                           tr_aero, tr_iage, tr_thickchng, tr_FY, &
                           tr_pond,                               & 
      ...
      use ice_thickchng, only: restart_thickchng
...
!
      ...
      namelist /tracer_nml/    &
        tr_iage, restart_age,  &
        tr_thickchng, restart_thickchng,  &
        tr_FY,   restart_FY,   &
        tr_lvl,  restart_lvl,  &
      ...
      filename_iage = 'none'
      tr_thickchng      = .true. ! ice thickchng 
      restart_thickchng  = .false. ! ice thickchng restart
      filename_thickchng = 'none'
...
!     call broadcast_array(histfreq_n(:),       master_task)
      ...
      call broadcast_scalar(tr_thickchng,             master_task)
      call broadcast_scalar(restart_thickchng,        master_task)
      ...
      if (my_task == master_task) then
         ...
         write(nu_diag,1010) ' tr_thickchng              = ', tr_thickchng
         write(nu_diag,1010) ' restart_thickchng         = ', restart_thickchng
         ...
         if (tr_thickchng) then
            nt_thickchng = ntrcr + 1
            ntrcr = ntrcr + 18 
         else
            nt_thickchng = max_ntrcr
         endif
 ...
 1050    format (a30,   6a8)   ! character
      ...
      call broadcast_scalar(nt_thickchng,  master_task)
...
!      if (tr_thickchng) trcr_depend(nt_thickchng)  = 1   ! volume ice 
      if (tr_thickchng) then   ! volume ice 
        do n=1,18
          trcr_depend(nt_thickchng+(n-1))  = 1   ! volume ice 
        enddo
      endif
