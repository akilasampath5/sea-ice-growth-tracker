...
!
      ...
      use ice_state, only: tr_aero, tr_iage, tr_thickchng, tr_FY,&
                           tr_pond, tr_lvl
      use ice_exit
!
...
!
      ...
      if (.not. tr_thickchng) f_thickchng   = 'xxxxx'
      ...
      call broadcast_scalar (f_thicmsk_N, master_task)
...
!     call broadcast_scalar (f_example, master_task)
      ...
      call broadcast_scalar (f_thickchng, master_task)
...
!!!!! end example
      ...
      ! Ice thickchng 
      if (f_thickchng(1:1) /= 'x') then 
         do n=1,18
            write(nchar,'(i3.3)') n
            write(vname_in,'(a,a)') 'thickchng', trim(nchar) ! flatn_ai
            stmp = 'ice volume, category '   ! vicen
            write(vdesc_in,'(a,2x,a)') trim(stmp), trim(nchar)
            call define_hist_field(n_thickchng(n,:),                 &
                 vname_in,"m", tstr, tcstr,                          &
                 vdesc_in, "none", c1, c0,                           &
              ns1, f_thickchng)
         enddo
      endif
...
!           call accum_hist_field(n_sig2, iblk, sig2(:,:,iblk))
        ...
        if (f_thickchng(1:1) /= 'x') then
           do n=1,18
              call accum_hist_field(n_thickchng(n,:), iblk, &
                                 trcr(:,:,nt_thickchng +1*(n-1),iblk))
           enddo
        endif
...
!                 if (n_thickchng(ns) /= 0) aa(i,j,n_thickchng(ns),iblk) = spval
...
!                 if (n_thickchng     (ns) /= 0) aa(i,j,n_thickchng(ns),iblk)      = &
!                       trcr(i,j,nt_thickchng,iblk)*avail_hist_fields(n_thickchng(ns))%cona
