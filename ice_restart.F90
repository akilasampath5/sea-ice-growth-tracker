...
!
      ...
      if (my_task == master_task) then
         ...
         write(nu_dump) filename_thickchng
...
!
      ...
      if (tr_thickchng) then
         do k=1,18
            write(nchar2,'(i2.2)') k
            call define_rest_field(File,'thickchng'//nchar2,dims)
         enddo
      end if
      ...
      if (tr_thickchng) then
         do k=1,18
            write(nchar2,'(i2.2)') k
            status = pio_inq_varid(File,'thickchng'//nchar2,varid)
            call pio_write_darray(File, varid, iodesc3d_ncat, trcrn(:,:,nt_thickchng+(k-1),:,:), status, fillval=c0)
         enddo
      endif
...
!
      ...
      if (tr_thickchng) trcrn(:,:,nt_thickchng, :,:) = c0
      ...
      if (my_task == master_task) then
         ...
         read(nu_restart, end=99) filename_thickchng
   ...
   99    continue
      ...
      if (my_task == master_task) then
         ...
         write(nu_diag,'(a,a)') 'filename_thickchng : ',filename_thickchng
      ...
      call broadcast_scalar(filename_thickchng,  master_task)
